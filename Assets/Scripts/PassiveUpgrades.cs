...
public class PassiveUpgrades : MonoBehaviour
{
    ...
    public static int[] passiveLvls = { 0, 0, 0, 0 };
    ...
    void Update()
    {
        ...
        {
            PlayerScript.playerCoins += 5000;
            pscript.UpdatePlayerCoinsText();
        }

        if (Input.GetKeyDown(KeyCode.M))
        {
            for(int i = 0; i < passiveLvls.Length; i++)
            {
                Debug.Log("Nivelul puterii " + i + "este" + passiveLvls[i]);
            }
        }
    }
    ...
    void Initialization()
    {
        ...
        for (int i = 0; i < items.Length; i++)
        {
            ...
            items[i].lvl = passiveLvls[items[i].id - 1];
        ...
        for (int i = 0; i < items.Length; i++)
        {
            items[i].cost = items[i].cost * (int)Math.Pow(2, passiveLvls[items[i].id - 1]);
