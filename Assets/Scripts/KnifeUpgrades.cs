...
public class KnifeUpgrades : MonoBehaviour
{
    ...
    public static int[] knifeLvls = { 0, 0, 0, 0, 0 };
    ...
    void Initialization()
    {
        ...
        for (int i = 0; i < items.Length; i++)
        {
            ...
            items[i].lvl = knifeLvls[items[i].id - 1];
        ...
        for (int i = 0; i < items.Length; i++)
        {
            items[i].cost = items[i].cost * (int)Math.Pow(2, knifeLvls[items[i].id - 1]);
